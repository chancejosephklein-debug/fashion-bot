    ...
    try:
        ...
        for brand, score in ranked[:10]:
            ...
            data.append({
                ...
                "stockx_ask": pd.get("stockx_ask"),
                "grailed_sold": pd.get("grailed_sold"),
            })
        return web.json_response({"trends": data, "sources": sources_ok, "ok": True})
...
async def price_cmd(ctx, *, item: str):
    ...
    async with aiohttp.ClientSession() as session:
        stockx, _ = await fetch_stockx(session, item)
        grailed, _ = await fetch_grailed(session, item)
    ...
    if stockx:
        ...
        for p in stockx[:3]:
            ...
            ask = p.get("lowestAsk") or p.get("price")
            lines.append(f"**{name}**\n  Lowest Ask: {fmt_price(ask)}")
    ...
    if grailed:
        ...
        for g in grailed[:3]:
            ...
            sold = g.get("soldPrice") or g.get("price")
            lines.append(f"**{name}**\n  Sold: {fmt_price(sold)}")
        embed.add_field(name="GRAILED (SOLD)", value="\n\n".join(lines), inline=False)

    if not stockx and not grailed:
        embed.description = "⚠ No listings found. Try different keywords."
